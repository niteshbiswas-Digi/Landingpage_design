...
function MagneticBtn({ children, primary, href }: { children: React.ReactNode; primary?: boolean; href?: string }) {
  ...
  return (
    <motion.a
      ...
      whileHover={primary ? { boxShadow: '0 16px 50px rgba(245,166,35,0.3)', scale: 1.03 } : { borderColor: 'rgba(245,166,35,0.4)', color: '#e0e0e0' }}
      whileTap={{ scale: 0.95 }}
    >
      {primary && (
        <motion.span
      ...
      >
        ...
        {!isMobile && (
          <div style={{ display: 'flex', gap: 36, alignItems: 'center' }}>
            {links.map((link, i) => {
              const active = link.label === 'Services';
              return (
        ...
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }}
          ...
          {links.map(link => (
            <a key={link.label} href={link.href} onClick={() => setMenuOpen(false)}
              style={{ display: 'block', padding: '12px 0', fontSize: 14, fontWeight: 600, color: link.label === 'Services' ? '#f5a623' : '#888', textDecoration: 'none', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
              {link.label}
            </a>
          ))}
...
const HERO_CARDS = [
  { title: 'Web Development',    sub: 'React · Next.js · Node.js',    accent: '#f5a623', icon: '⬡' },
  ...
  return (
    <section style={{
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 3 }}
        style={{ position: 'absolute', top: '-20%', left: '50%', transform: 'translateX(-50%)', width: 1100, height: 800, background: 'radial-gradient(ellipse, rgba(245,166,35,0.08) 0%, transparent 60%)', pointerEvents: 'none' }} />
      ...
      <div style={{
        ...
        <div>
          ...
          >
            ...
            <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.44em', textTransform: 'uppercase', color: '#f5a623' }}>What We Build</span>
          </motion.div>
          ...
          <h1 style={{ fontSize: isMobile ? 'clamp(46px,11vw,72px)' : 'clamp(56px,7.5vw,96px)', fontWeight: 900, letterSpacing: '-0.055em', lineHeight: 0.87, margin: '0 0 32px' }}>
            {[
              ...
              { text: 'Services.',  style: { color: '#f5a623' } },
            ].map((line, i) => (
          ...
          >
            {STATS.map((s, i) => (
              <div key={i} style={{
                ...
                <div style={{ fontSize: isMobile ? 22 : 26, fontWeight: 900, color: '#f5a623', letterSpacing: '-0.05em', lineHeight: 1, fontFamily: 'var(--font-outfit)', marginBottom: 5 }}>
                  {s.value}
                </div>
          ...
          <div style={{ position: 'relative' }}>
            ...
            <motion.div
              ...
              style={{
                ...
                background: 'rgba(8,8,8,0.96)', border: '1px solid rgba(245,166,35,0.22)',
            ...
            >
              <div style={{ fontSize: 26, fontWeight: 900, color: '#f5a623', letterSpacing: '-0.05em', lineHeight: 1 }}>150+</div>
...
function SectionLabel({ text, inView, center = false }: { text: string; inView: boolean; center?: boolean }) {
  return (
    ...
    >
      {center && <motion.div initial={{ scaleX: 0 }} animate={inView ? { scaleX: 1 } : {}} transition={{ duration: 0.5, ease: E }} style={{ width: 32, height: 1, background: '#f5a623', transformOrigin: 'right' }} />}
      <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.44em', textTransform: 'uppercase', color: '#f5a623' }}>{text}</span>
      <motion.div initial={{ scaleX: 0 }} animate={inView ? { scaleX: 1 } : {}} transition={{ duration: 0.5, ease: E }} style={{ width: 32, height: 1, background: '#f5a623', transformOrigin: 'left' }} />
    </motion.div>
  );
}
...
const WEB_SERVICES = [
  ...
  { iconKey: 'web',       title: 'Website Development Services', accent: '#f5a623', body: 'Comprehensive website development — from pixel-perfect front ends to fully connected admin panels and content management systems.' },
...
function WebSection() {
  ...
  return (
    <section id="web" ref={ref} style={{
      ...
      <div className="orb-a" style={{ position: 'absolute', top: '-12%', right: '-6%', width: 600, height: 600, borderRadius: '50%', background: 'radial-gradient(circle, rgba(245,166,35,0.05) 0%, transparent 65%)', filter: 'blur(60px)', pointerEvents: 'none' }} />
...
const MOBILE_SERVICES = [
  ...
  { iconKey: 'api',     title: 'API Integration Solutions',   accent: '#f5a623', body: 'Seamlessly connect systems and services. We design and integrate RESTful and GraphQL APIs for streamlined operations, improved data flow, and scalable functionality.' },
];
...
const STACKS = [
  ...
  { label: 'Back End',      items: ['Python Django', 'FastAPI', 'Node.js', 'PHP Laravel'],         accent: '#f5a623' },
...
function TechStackSection() {
  ...
  return (
    <section ref={ref} style={{
      ...
      <div className="orb-a" style={{ position: 'absolute', top: '5%', right: '-6%', width: 550, height: 550, borderRadius: '50%', background: 'radial-gradient(circle, rgba(245,166,35,0.045) 0%, transparent 65%)', filter: 'blur(60px)', pointerEvents: 'none' }} />
    ...
    <section id="contact" ref={ref} style={{
      ...
      <div style={{ position: 'relative', zIndex: 10, maxWidth: 720, margin: '0 auto', textAlign: 'center' }}>
        ...
        >
          ...
          <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.44em', textTransform: 'uppercase', color: '#f5a623' }}>Ready to innovate?</span>
          <motion.div initial={{ scaleX: 0 }} animate={inView ? { scaleX: 1 } : {}} transition={{ duration: 0.5, delay: 0.08, ease: E }}
        ...
        <h2 style={{ fontSize: isMobile ? 'clamp(38px,9vw,56px)' : 'clamp(48px,7vw,84px)', fontWeight: 900, letterSpacing: '-0.055em', lineHeight: 0.9, margin: '0 0 36px' }}>
          ...
          <br />
          <WordReveal text="UpCodo" inView={inView} delay={0.22} style={{ color: '#f5a623' }} />
        </h2>
